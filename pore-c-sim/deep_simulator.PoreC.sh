...
out_root=""
#------- optioanl parameters -----------#
THREAD_NUM=4        #-> this is the thread (or, CPU) number
...
EVENT_STD=1      #-> set the std of random noise of the event, default = 1.0
...
FILTER_FREQ=840     #-> set the frequency for the low-pass filter. default = 950
NOISE_STD=0       #-> set the std of random noise of the signal, default = 1.0
#-> perfect mode
...
## one chrom one file processed_genome_0 ... processed_genome_n
...
## origin1: one processed_genome_0 one sampled_read_processed_genome_0.fasta, header like ">2"
## origin2: merge all sampled_read_processed_genome_0.fasta to one sampled_read.fasta
## PoreC1: 3C_info,   header like ">p0_chr1_0423 chr4-120509899-120554814-16-0;chr1-376889-402806-16-0"
## PoreC2: 3C_id,     header like ">p0_chr1_0423"
## PoreC3: sampled_id,header like ">2", the same as original sampled_read.fasta
conda activate basecall
echo "Loading 3C simulated reads..."
awk '{if($0~/>/){sub(/ .*/, "", $0); print$0}else{print$0}}' $FULLFILE > $FILENAME/sampled_read_processed_genome_0.fasta
seqtk rename $FILENAME/sampled_read_processed_genome_0.fasta > $FILENAME/sampled_read.fasta
> $FILENAME/sampled_read_processed_genome_0.fasta
conda deactivate
...
then
	...
	$home/base_caller/$guppy/ont-guppy-cpu/bin/guppy_basecaller -r --input_path $FAST5_DIR \
		--save_path $FASTQ_DIR -c dna_r9.4.1_450bps_hac.cfg \
		--cpu_threads_per_caller $THREAD_NUM --num_callers 1 --disable_pings
else
...
# output Csim.fastq
conda activate basecall
echo "Matching 3C simulated information and DeepSimulator results..."
python $home/util/Csim_matchup.py \
	-ca $FULLFILE \
	-dq $FILENAME/pass.fastq \
	-dp $FILENAME/mapping.paf \
	-cq $FILENAME/Csim.fastq 
conda deactivate
echo "Finished."
#---------- exit -----------#
exit 0
